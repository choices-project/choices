...
- [ ] **`npm run verify:docs`** (repository root) passes when `web/app/api/**/route.ts` changed; run **`npm run docs:api-inventory`** first if the inventory needs regenerating. When **`web/lib/core/feature-flags.ts`** changes, run **`npm run docs:feature-flags`** before `verify:docs`.
