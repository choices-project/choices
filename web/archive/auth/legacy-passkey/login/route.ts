...
export async function POST(request: NextRequest) {
  try {
    ...
    const { credential, challenge, username, displayName } = body;
    ...
    if (credential) {
      ...
      console.log(`Passkey verification attempt for user: ${username}${displayName ? ` (${displayName})` : ''}, challenge: ${challenge}`);
...
async function generateAuthOptionsHandler(request: NextRequest, body: any) {
  ...
  // Log authentication options request for audit trail
  console.log(`Authentication options requested for user: ${username}${displayName ? ` (${displayName})` : ''}`);
...
async function verifyAuthentication(request: NextRequest, body: any) {
  ...
  // Log verification attempt with display name if available
  console.log(`Verifying passkey for user: ${username}${displayName ? ` (${displayName})` : ''}`);
