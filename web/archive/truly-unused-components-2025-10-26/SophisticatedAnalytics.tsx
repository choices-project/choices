...
export function SophisticatedAnalytics({ pollId, trustTiers }: SophisticatedAnalyticsProps) {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // Use unified analytics endpoint with comprehensive analysis
      const trustTiersParam = trustTiers && trustTiers.length > 0 ? trustTiers.join(',') : '1,2,3';
      const unifiedUrl = `/api/analytics/unified/${pollId}?methods=comprehensive&ai-provider=colab&trust-tiers=${trustTiersParam}&analysis-window=24 hours&cache=true`;
      
      console.log('🔍 Fetching unified analytics:', unifiedUrl);
      ...
      // Extract analytics data from unified response
      const analytics = result.analytics;
      
      // Build combined data structure compatible with existing component
      const combinedData = {
        sentiment_analysis: analytics.comprehensive?.sentiment_analysis || {
          tier_breakdown: {},
          narrative_divergence: {
            score: 0,
        ...
        bot_detection: analytics.comprehensive?.bot_detection || {
        ...
        temporal_analysis: analytics.comprehensive?.temporal_analysis || {
          voting_patterns: {
            peak_hours: [],
        ...
        geographic_insights: analytics.comprehensive?.geographic_insights || {
          country_distribution: {},
          state_distribution: {},
          city_distribution: {}
        },
        trust_tier_results: analytics.comprehensive?.trust_tier_results || {},
        platform: result.metadata?.platform || 'choices',
        repository: result.metadata?.repository || 'https://github.com/choices-project/choices',
        live_site: result.metadata?.live_site || 'https://choices-platform.vercel.app',
        analysis_method: result.metadata?.analysis_method || 'unified_comprehensive',
        timestamp: result.metadata?.timestamp || new Date().toISOString(),
        cache_status: {
          unified: result.performance?.cache_hits > 0,
          total_time_ms: result.performance?.total_time_ms || 0,
          methods_successful: result.methods_successful || [],
          ai_provider: result.ai_provider || 'colab'
        },
        // Include unified analytics metadata
        unified_analytics: {
          methods_requested: result.methods_requested,
          methods_successful: result.methods_successful,
      ...
      console.log('✅ Unified analytics loaded successfully:', {
        methods: result.methods_successful,
        totalTime: result.performance?.total_time_ms,
        aiProvider: result.ai_provider,
        cacheHits: result.performance?.cache_hits
      });
