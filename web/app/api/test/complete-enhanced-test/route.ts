...
export async function GET(request: NextRequest) {
  try {
    ...
    // Test with a real federal candidate who has FEC data
    const testRep = {
      id: 'test-complete-fec',
    ...
    console.log('🧪 Testing complete enhanced pipeline with FEC data...');
    ...
    // Store with enhanced schema including FEC data
    const { error } = await supabase
      ...
      .upsert({
        ...
        // Campaign finance data (FEC) - stored in separate table
    ...
    return NextResponse.json({
      status: 'success',
      message: 'Complete enhanced pipeline test completed',
      data: {
        enriched: {
          name: enrichedRep.name,
          primaryEmail,
    ...
    return NextResponse.json({ 
      error: `Complete enhanced test failed: ${error}`,
      status: 'failed'
    }, { status: 500 });
  }
}
