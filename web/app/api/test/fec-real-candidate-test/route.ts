...
export async function GET(request: NextRequest) {
  try {
    ...
    // Test with a real federal candidate who should have FEC data
    const testRep = {
      id: 'test-fec-real',
      name: 'Donald Trump', // Real federal candidate with FEC data
      party: 'Republican',
      office: 'President',
    ...
    console.log('🧪 Testing FEC with real federal candidate (Trump)...');
    ...
    return NextResponse.json({ 
      error: `FEC real candidate test failed: ${error}`,
      status: 'failed'
    }, { status: 500 });
  }
}
