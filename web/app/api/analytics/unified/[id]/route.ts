  ...
  try {
    ...
    for (const method of validMethods) {
      ...
      try {
        ...
        const cacheKey = `unified_analytics_${pollId}_${method}_${aiProvider}_${trustTiers.join(',')}`;
        
        let result;
        if (cacheEnabled) {
          // For now, execute directly without caching
          result = await executeAnalyticsMethod(
            supabase,
            pollId,
            method,
            aiProvider,
...
async function executeBotDetection(
  ...
  try {
    ...
    const fallbackProvider = AI_PROVIDERS[aiProvider].fallback;
    if (fallbackProvider && fallbackProvider !== aiProvider) {
      return await executeBotDetection(supabase, pollId, fallbackProvider, trustTiers, analysisWindow, poll);
    }
  }
...
async function executeTemporalAnalysis(
  ...
  const recentVotes = votes.filter((vote: any) => new Date(vote.created_at) > oneDayAgo);
