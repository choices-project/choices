  ...
  const handleSocialAuth = async (provider: 'google' | 'github' | 'facebook' | 'twitter' | 'linkedin' | 'discord' | 'instagram' | 'tiktok') => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo: `${window.location.origin}/auth/callback?redirectTo=/feed`
        }
      });
        ...
        <div className="text-center">
          ...
          <input type="hidden" name="csrf-token" value="test-csrf-token" data-testid="csrf-token" />
              ...
              <div
                ...
                className={`bg-yellow-50 border border-yellow-200 rounded-md p-4 transition-all duration-200 ${
                  isRateLimited ? 'opacity-100 visible' : 'opacity-0 invisible h-0 p-0 border-0 overflow-hidden'
                }`}
                role="alert"
                aria-live="assertive"
                aria-atomic="true"
                ...
                <div className="flex">
                  ...
                  <div className="ml-3 flex-1">
                    <p className="text-sm font-medium text-yellow-800">
                      {userError && isRateLimited ? userError : t('auth.form.rateLimited') || 'Too many requests. Please try again later.'}
                    </p>
            ...
            <div className="mt-6 space-y-3">
              {availableProviders.map((provider) => (
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full inline-flex justify-center items-center py-2.5 px-4 border rounded-md shadow-sm text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed ${provider.bgColor} ${provider.borderColor} ${provider.textColor} ${provider.hoverBgColor} ${provider.hoverTextColor}`}
