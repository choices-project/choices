...
test.describe('Upgraded Dashboards E2E Test', () => {
  test('should test upgraded personal and admin dashboards with comprehensive functionality', async ({ page, baseURL, request }) => {
    ...
    EnhancedDatabaseTracker.initializeSupabase(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
    ...
    try {
      ...
      if (userAnalyticsResponse.status() === 200) {
        ...
        EnhancedDatabaseTracker.trackQuery('user_profiles', 'select', 'personal_dashboard_analytics');
        EnhancedDatabaseTracker.trackQuery('votes', 'select', 'personal_dashboard_analytics');
        EnhancedDatabaseTracker.trackQuery('polls', 'select', 'personal_dashboard_analytics');
        EnhancedDatabaseTracker.trackQuery('feed_interactions', 'select', 'personal_dashboard_analytics');
      } else {
    ...
    try {
      ...
      if (adminDashboardResponse.status() === 200) {
        ...
        EnhancedDatabaseTracker.trackQuery('user_profiles', 'select', 'admin_dashboard_api');
        EnhancedDatabaseTracker.trackQuery('polls', 'select', 'admin_dashboard_api');
        EnhancedDatabaseTracker.trackQuery('votes', 'select', 'admin_dashboard_api');
        EnhancedDatabaseTracker.trackQuery('feedback', 'select', 'admin_dashboard_api');
      } else {
    ...
    try {
      ...
      if (adminAnalyticsResponse.status() === 200) {
        ...
        EnhancedDatabaseTracker.trackQuery('analytics_events', 'select', 'admin_analytics_api');
        EnhancedDatabaseTracker.trackQuery('trust_tier_analytics', 'select', 'admin_analytics_api');
        EnhancedDatabaseTracker.trackQuery('analytics_user_engagement', 'select', 'admin_analytics_api');
      } else {
    ...
    try {
      ...
      if (siteMessagesResponse.status() === 200) {
        ...
        EnhancedDatabaseTracker.trackQuery('site_messages', 'select', 'admin_site_messages_api');
      } else {
    ...
    await EnhancedDatabaseTracker.saveReport('upgraded-dashboards-e2e-test.json');
