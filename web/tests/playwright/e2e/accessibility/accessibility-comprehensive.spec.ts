...
import { runComprehensiveAccessibilityTests, generateAccessibilityReport } from './accessibility-utils';
...
test.describe('Comprehensive Accessibility Tests', () => {
  ...
  test('should pass all accessibility tests', async ({ page }) => {
    ...
    const metrics = await runComprehensiveAccessibilityTests(page);
    
    // Generate and log accessibility report
    const report = generateAccessibilityReport(metrics);
    console.log(report);
    ...
    const criticalIssues = metrics.overall.violations.filter(violation => 
