...
test.describe('Authenticated API Database Test', () => {
  test('should test APIs with authentication and track database usage', async ({ page }) => {
    ...
    const healthResult = await ApiHelper.get(page, '/api/health');
    if (healthResult.success) {
      DatabaseTracker.trackQuery('system_health', 'select', 'health_api_authenticated');
    }
    
    // Test Polls API with authentication
    console.log('🗳️ Testing Polls API with authentication');
    const pollsResult = await ApiHelper.get(page, '/api/polls');
    if (pollsResult.success) {
      DatabaseTracker.trackQuery('polls', 'select', 'polls_api_authenticated');
      DatabaseTracker.trackQuery('votes', 'select', 'polls_api_authenticated');
      DatabaseTracker.trackQuery('poll_analytics', 'select', 'polls_api_authenticated');
    }
    
    // Test Hashtags API with authentication
    console.log('🏷️ Testing Hashtags API with authentication');
    const hashtagsResult = await ApiHelper.get(page, '/api/hashtags');
    if (hashtagsResult.success) {
      DatabaseTracker.trackQuery('hashtags', 'select', 'hashtags_api_authenticated');
      DatabaseTracker.trackQuery('hashtag_usage', 'select', 'hashtags_api_authenticated');
      DatabaseTracker.trackQuery('hashtag_trends', 'select', 'hashtags_api_authenticated');
    }
    ...
    try {
      ...
      DatabaseTracker.trackQuery('analytics_events', 'select', 'analytics_api_authenticated');
      DatabaseTracker.trackQuery('trust_tier_analytics', 'select', 'analytics_api_authenticated');
      DatabaseTracker.trackQuery('analytics_user_engagement', 'select', 'analytics_api_authenticated');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('user_profiles', 'select', 'profile_api_authenticated');
      DatabaseTracker.trackQuery('private_user_data', 'select', 'profile_api_authenticated');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('user_profiles', 'select', 'dashboard_api_authenticated');
      DatabaseTracker.trackQuery('feeds', 'select', 'dashboard_api_authenticated');
      DatabaseTracker.trackQuery('feed_interactions', 'select', 'dashboard_api_authenticated');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('feeds', 'select', 'feeds_api_authenticated');
      DatabaseTracker.trackQuery('feed_interactions', 'select', 'feeds_api_authenticated');
      DatabaseTracker.trackQuery('trending_topics', 'select', 'feeds_api_authenticated');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('feedback', 'select', 'feedback_api_authenticated');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('representatives_core', 'select', 'civics_api_authenticated');
      DatabaseTracker.trackQuery('representative_contacts_optimal', 'select', 'civics_api_authenticated');
      DatabaseTracker.trackQuery('representative_offices_optimal', 'select', 'civics_api_authenticated');
      DatabaseTracker.trackQuery('user_address_lookups', 'insert', 'civics_api_authenticated');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('user_address_lookups', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('state_districts', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('zip_to_ocd', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('latlon_to_ocd', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('geographic_lookups', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('votes', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('polls', 'select', 'heatmap_api_authenticated');
      DatabaseTracker.trackQuery('user_profiles', 'select', 'heatmap_api_authenticated');
    } catch (error) {
    ...
    const verifiedTables = DatabaseTracker.getVerifiedTables();
    const queryLog = DatabaseTracker.getQueryLog();
    const dataVerification = DatabaseTracker.getDataVerification();
    ...
    await DatabaseTracker.saveReport('authenticated-api-database-test.json');
  ...
  test('should test admin APIs with admin authentication', async ({ page }) => {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('user_profiles', 'select', 'admin_users_api');
      DatabaseTracker.trackQuery('admin_activity_log', 'select', 'admin_users_api');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('feedback', 'select', 'admin_feedback_api');
      DatabaseTracker.trackQuery('admin_activity_log', 'select', 'admin_feedback_api');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('audit_logs', 'select', 'admin_system_status_api');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('system_health', 'select', 'admin_system_metrics_api');
      DatabaseTracker.trackQuery('audit_logs', 'select', 'admin_system_metrics_api');
    } catch (error) {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('system_health', 'select', 'admin_performance_api');
      DatabaseTracker.trackQuery('audit_logs', 'select', 'admin_performance_api');
    } catch (error) {
    ...
    // Save comprehensive report
    await DatabaseTracker.saveReport('admin-api-database-test.json');
