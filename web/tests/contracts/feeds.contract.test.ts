/**
 * Feeds API Contract Tests
 *
 * Verifies critical feeds endpoint behavior: response shape, rate limiting,
 * and error handling. Uses mocked Supabase.
 *
...
describe('Feeds API contract', () => {
  ...
  beforeEach(() => {
    ...
    (mockRateLimiter.checkLimit as jest.Mock).mockResolvedValue({ allowed: true });
    (mockLogger.warn as jest.Mock).mockReset();
  });

  it('returns feed items with correct response shape', async () => {
    ...
    (mockSupabaseClient.from as jest.Mock).mockImplementation((table: string) => {
      ...
      if (table === 'civic_actions') return civicActionsBuilder;
  ...
  it('enforces rate limit and returns 429 when exceeded', async () => {
    (mockRateLimiter.checkLimit as jest.Mock).mockResolvedValue({ allowed: false });
  ...
  it('returns 500 when polls query fails', async () => {
    const failingPolls = createPostgrestBuilder({
      data: [],
      error: { message: 'poll query failed' } as Error,
    });

    (mockSupabaseClient.from as jest.Mock).mockImplementation((table: string) => {
      if (table === 'polls') return failingPolls;
  ...
  it('returns polls when civic actions query fails and logs warning', async () => {
    const pollsBuilder = createPostgrestBuilder({
      data: [
        {
          id: 'poll-1',
    ...
    const civicActionsBuilder = createPostgrestBuilder({
      data: [],
      error: { message: 'civic actions failed' } as Error,
    });
