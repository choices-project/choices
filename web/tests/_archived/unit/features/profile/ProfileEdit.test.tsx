...
type ProfileValue = Exclude<ProfileEditProps['profile'], undefined>;

const createProfile = (overrides: Partial<ProfileEditProps['profile']> = {}): ProfileValue =>
  ({
    id: 'profile-1',
  ...
  }) as unknown as ProfileValue;

const renderComponent = (props: Partial<ProfileEditProps> = {}) => {
  const profile: ProfileValue =
    props.profile === undefined ? createProfile() : (props.profile as ProfileValue);
...
describe('ProfileEdit', () => {
  ...
  it('binds inputs to user store draft and submits typed payload', async () => {
    ...
    if (!profile) {
      throw new Error('Expected profile to be set for cancel flow');
    }
