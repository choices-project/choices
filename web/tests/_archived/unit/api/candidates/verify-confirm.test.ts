...
interface MockQueryChain {
  select: MockFn<(...args: any[]) => MockQueryChain>;
  eq: MockFn<(...args: any[]) => MockQueryChain>;
  order: MockFn<(...args: any[]) => MockQueryChain>;
  limit: MockFn<(...args: any[]) => MockQueryChain>;
  maybeSingle: MockFn<(...args: any[]) => Promise<unknown>>;
  update: MockFn<(...args: any[]) => Promise<unknown>>;
  ilike: MockFn<(...args: any[]) => MockQueryChain>;
  or: MockFn<(...args: any[]) => MockQueryChain>;
}

const createMockQueryChain = (): MockQueryChain => {
  const chain = {} as MockQueryChain;
  const chainMethod = () =>
    (jest.fn() as jest.MockedFunction<(...args: any[]) => MockQueryChain>).mockReturnValue(chain);

  chain.select = chainMethod();
  chain.eq = chainMethod();
  chain.order = chainMethod();
  chain.limit = chainMethod();
  chain.update = jest.fn() as jest.MockedFunction<(...args: any[]) => Promise<unknown>>;
  chain.ilike = chainMethod();
  chain.or = chainMethod();
  chain.maybeSingle = jest.fn() as jest.MockedFunction<(...args: any[]) => Promise<unknown>>;
...
type MockSupabaseClient = {
  auth: {
    getUser: MockFn<() => Promise<SupabaseAuthResponse>>;
  };
  from: MockFn<(table: string) => MockQueryChain>;
};
...
const createMockSupabaseClient = (options?: {
  ...
  return {
    auth: {
      getUser: (jest.fn() as jest.MockedFunction<() => Promise<SupabaseAuthResponse>>).mockResolvedValue({
          data: { user: options?.authUser ?? DEFAULT_AUTH_USER },
          error: null
        })
    },
    from: (jest.fn() as jest.MockedFunction<(table: string) => MockQueryChain>).mockImplementation(
      (table: string) => fromImplementation(table),
    ),
  };
};
...
const resolveSupabaseClient = async (client: MockSupabaseClient) => {
  ...
  jest
    ...
    .mockResolvedValue(client as unknown as SupabaseServerClient);
};
...
describe('POST /api/candidates/verify/confirm - Wrong Code Handling', () => {
  ...
  beforeEach(async () => {
    ...
    mockRequest = {
      ...
      json: (jest.fn() as jest.MockedFunction<() => Promise<unknown>>).mockResolvedValue({
        code: '999999',
      }) // Valid numeric code that doesn't match
    } as unknown as MockNextRequest;
  });
...
describe('POST /api/candidates/verify/confirm - Input Validation', () => {
  ...
  it('should reject request with missing code', async () => {
    mockRequest.json = (jest.fn() as jest.MockedFunction<() => Promise<unknown>>).mockResolvedValue({});
  ...
  it('should reject request with empty code', async () => {
    mockRequest.json = (jest.fn() as jest.MockedFunction<() => Promise<unknown>>).mockResolvedValue({ code: '' });
