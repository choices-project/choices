...
describe('appStore', () => {
  ...
  it('removeBreadcrumb removes breadcrumb at index', () => {
    ...
    expect(store.getState().breadcrumbs).toHaveLength(2);
    const breadcrumb = store.getState().breadcrumbs[1];
    if (!breadcrumb) {
      throw new Error('Expected a breadcrumb at index 1');
    }
    expect(breadcrumb.label).toBe('Detail');
  });
...
describe('appStore persistence', () => {
  ...
  const createPersistedTestStore = () => {
    ...
    return create<AppStore>()(
      persist(immer(appStoreCreator as any), {
        name: 'app-store',
