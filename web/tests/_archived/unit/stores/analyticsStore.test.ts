...
const createTestAnalyticsStore = () =>
  create<AnalyticsStore>()(immer(analyticsStoreCreator as any));
...
describe('analyticsStore', () => {
  ...
  describe('consent guard', () => {
    ...
    it('trackPageView respects consent guard', () => {
      ...
      if (!firstEvent) {
        throw new Error('Expected a page view event');
      }
      expect(firstEvent.type).toBe('page_view');
      expect(firstEvent.label).toBe('/test-page');
    });
    ...
    it('trackUserAction respects consent guard', () => {
      ...
      const firstEvent = store.getState().events[0];
      if (!firstEvent) {
        throw new Error('Expected a user action event');
      }
      expect(firstEvent.type).toBe('user_action');
      expect(firstEvent.action).toBe('click');
      expect(firstEvent.category).toBe('button');
      expect(firstEvent.label).toBe('submit-btn');
    });
    ...
    it('trackError respects consent guard', () => {
      ...
      expect(firstEvent.label).toBe('Test error');
    });
