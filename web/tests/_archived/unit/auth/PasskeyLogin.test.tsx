...
describe('PasskeyLogin', () => {
  const mockedBeginAuthenticate = jest.mocked(beginAuthenticate);
  ...
  it('shows success message when authentication succeeds', async () => {
    mockedBeginAuthenticate.mockResolvedValue({ success: true, data: {} });
  ...
  it('shows error message when authentication fails', async () => {
    mockedBeginAuthenticate.mockResolvedValue({ success: false, error: 'Authentication failed' });
