    ...
    test('critical resources load first', async ({ page }) => {
      ...
      if (cssResources.length > 0) {
        ...
        if (jsResources.length > 0) {
          const firstCss = cssResources[0];
          const firstJs = jsResources[0];
          if (!firstCss || !firstJs) {
            return;
          }
          const firstCSSIndex = resourceLoadOrder.indexOf(firstCss);
          const firstJSIndex = resourceLoadOrder.indexOf(firstJs);
