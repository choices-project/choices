...
describe('Network Effects', () => {
  ...
  test('should enforce exposure caps', async () => {
    const capManager = new ExposureCapManager();
    ...
    jest.spyOn(ExposureCapManager as any, 'getDiversityExposureCount').mockResolvedValue(4);
    ...
    expect(canShowDiversity).toBe(false); // 4 >= 5, should hit cap
  });
