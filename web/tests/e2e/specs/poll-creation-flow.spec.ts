...
test.describe('Poll Creation Flow', () => {
  ...
  test('should publish poll successfully', async ({ page }) => {
    ...
    // Click publish - use more specific selector to avoid matching step indicator
    const publishButton = page.locator('footer button:has-text("Publish")').or(page.locator('button[type="button"]:has-text("Publish"):not([aria-label*="step"])'));
