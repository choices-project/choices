...
const gotoHarness = async (page: Page) => {
  await page.goto('/e2e/profile-store', {
    waitUntil: 'domcontentloaded',
    timeout: 60_000,
  });
  await waitForPageReady(page);
  await page.waitForFunction(
    () => Boolean(window.__profileStoreHarness),
    { timeout: 60_000 },
  );
};
...
test.describe('Profile store harness', () => {
  test('exposes core state transitions', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      } as Parameters<ProfileStoreHarness['setProfile']>[0]);
    });
    ...
    await page.evaluate(() => {
      ...
      } as unknown as Parameters<ProfileStoreHarness['setUserProfile']>[0]);
    });
