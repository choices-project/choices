...
const gotoHarness = async (page: Page) => {
  await page.goto('/e2e/pwa-store', {
    waitUntil: 'domcontentloaded',
    timeout: 60_000,
  });
  await waitForPageReady(page);
  await page.waitForFunction(
    () => Boolean(window.__pwaStoreHarness),
    { timeout: 60_000 },
  );
  try {
    await page.waitForFunction(
    ...
    // Harness is available; ready attr may not be set if persistence hasn't hydrated
  }
};

test.describe('@axe PWA store harness', () => {
  test('drives install, offline queue, notifications, and reset flows', async ({
    page,
  }) => {
