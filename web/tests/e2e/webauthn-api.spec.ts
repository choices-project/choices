...
test.describe('WebAuthn API Endpoints - V2', () => {
  ...
  test('should handle WebAuthn API with different user types with V2 setup', async ({ page }) => {
    ...
    const regularResponse = await page.request.post('/api/v1/auth/webauthn/register/options', {
      headers: {
    ...
    const adminResponse = await page.request.post('/api/v1/auth/webauthn/register/options', {
      headers: {
