/**
 * IA/PO Onboarding Completion Debug Test
 * Debug the onboarding completion process
 */

import { test, expect } from '@playwright/test'

test.describe('IA/PO Onboarding Completion Debug', () => {
  test('Debug onboarding completion step by step', async ({ page }) => {
    console.log('=== ONBOARDING COMPLETION DEBUG TEST ===')
    ...
    // Step 3: Check session cookie before onboarding
    console.log('🍪 Step 3: Check session cookie before onboarding')
    const cookiesBefore = await page.context().cookies()
    const sessionCookieBefore = cookiesBefore.find(c => c.name === 'choices_session')
    
    if (sessionCookieBefore) {
      console.log('✅ Session cookie found before onboarding')
    } else {
      console.log('❌ No session cookie found before onboarding')
      throw new Error('No session cookie before onboarding')
    }
    
    // Step 4: Complete onboarding steps
    console.log('🎯 Step 4: Complete onboarding steps')
    ...
    // Check session cookie before clicking
    ...
    console.log(`Session cookie before click: ${sessionCookieBeforeClick ? 'present' : 'missing'}`)
    
    // Click Get Started
    await getStartedButton.click()
    console.log('✅ Get Started button clicked')
    
    // Step 5: Wait for redirect and check what happens
    console.log('🎯 Step 5: Wait for redirect')
    await page.waitForTimeout(5000)
    ...
    // Check session cookie after clicking
    ...
    console.log(`Session cookie after click: ${sessionCookieAfterClick ? 'present' : 'missing'}`)
    ...
    await page.screenshot({ path: 'test-results/onboarding-completion-debug.png', fullPage: true })
    
    // Log all the information
    console.log('=== ONBOARDING COMPLETION DEBUG SUMMARY ===')
    ...
    console.log(`Redirected to register: ${urlAfterClick.includes('/register')}`)
    console.log(`Redirected to login: ${urlAfterClick.includes('/login')}`)
    console.log(`Session cookie before: ${sessionCookieBeforeClick ? 'present' : 'missing'}`)
    console.log(`Session cookie after: ${sessionCookieAfterClick ? 'present' : 'missing'}`)
