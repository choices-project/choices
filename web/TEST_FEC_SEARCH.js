// Test FEC API search directly
require('dotenv').config({ path: '.env.local' });

async function testFECSearch() {
  const apiKey = process.env.FEC_API_KEY;
  console.log('Testing FEC API search...');
  
  try {
    // Test the exact search that the pipeline would use
    ...
    const searchUrl = `https://api.open.fec.gov/v1/candidates/?name=${encodeURIComponent(searchName)}&state=${state}${officeParam}&api_key=${apiKey}&election_year=2024&election_year=2022&election_year=2020&candidate_status=C`;
