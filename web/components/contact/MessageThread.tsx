...
import { contactMessagingService, type Message } from '@/lib/contact/real-time-messaging';
...
export function MessageThread({ 
  ...
  useEffect(() => {
    ...
    const handleError = (err: Error) => {
      ...
      logger.error('Real-time message error', err, { threadId, error: err });
    };

    const channel = contactMessagingService.subscribeToThreadMessages(
      threadId,
      handleNewMessage,
      handleError
    );
    ...
    return () => {
      contactMessagingService.unsubscribe(`thread_messages_${threadId}`);
    };
  }, [threadId]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const result = await contactMessagingService.sendMessage({
        threadId,
        representativeId: parseInt(representative.id),
        subject: 'Reply', // Thread messages don't need subjects
