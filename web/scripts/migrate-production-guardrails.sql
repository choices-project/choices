-- Production Guardrails Migration - Run this in Supabase SQL Editor
-- This handles existing data properly to avoid unique constraint violations

-- Step 1: Add columns to representatives table
ALTER TABLE public.civics_representatives 
ADD COLUMN IF NOT EXISTS person_id UUID,
ADD COLUMN IF NOT EXISTS source TEXT,
ADD COLUMN IF NOT EXISTS external_id TEXT,
ADD COLUMN IF NOT EXISTS data_origin TEXT DEFAULT 'api',
ADD COLUMN IF NOT EXISTS valid_from TIMESTAMPTZ DEFAULT NOW(),
ADD COLUMN IF NOT EXISTS valid_to TIMESTAMPTZ DEFAULT 'infinity'::timestamptz;

-- Step 2: Update existing federal representatives
...
-- Step 3: Update existing state representatives
UPDATE public.civics_representatives 
SET 
...
-- Step 4: Update existing local representatives
UPDATE public.civics_representatives 
SET 
  source = CASE 
...
-- Step 5: Handle any remaining duplicates by making external_id unique
UPDATE public.civics_representatives 
...
-- Step 6: Create canonical crosswalk table
...
-- Step 7: Add unique constraint (now that data is clean)
...
-- Step 9: Create source precedence table
...
-- Step 10: Create expected counts table
...
-- Step 11: Create quality thresholds table
...
-- Step 12: Enable RLS (only for existing tables)
ALTER TABLE public.civics_representatives ENABLE ROW LEVEL SECURITY;

-- Step 13: Create RLS policies (only for existing tables)
CREATE POLICY "Public read access for representatives" ON public.civics_representatives
  FOR SELECT USING (true);

-- Note: RLS for civics_contact_info and civics_social_engagement will be enabled
-- when those tables are created in future migrations

-- Step 14: Create public view (only using existing columns)
...
-- Step 15: Create count drift function
...
-- Step 16: Create minimal FEC table
...
-- Step 17: Create minimal voting records table
...
-- Step 19: Create update trigger function
...
-- Migration complete!
-- You can now test the production guardrails
