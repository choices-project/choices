...
-- Implements user-controlled encryption and consent management
...
-- Only system admin can access privacy logs
...
-- Consent indexes
CREATE INDEX idx_user_consent_user_id ON user_profiles_encrypted(user_id);
CREATE INDEX idx_user_consent_type ON user_consent(consent_type);
CREATE INDEX idx_user_consent_active ON user_consent(user_id, consent_type, granted, revoked_at);

-- Analytics indexes
CREATE INDEX idx_analytics_contributions_poll_id ON analytics_contributions(poll_id);
CREATE INDEX idx_analytics_contributions_user_id ON analytics_contributions(user_id);
CREATE INDEX idx_analytics_contributions_consent ON analytics_contributions(consent_granted);

-- Privacy logs indexes
CREATE INDEX idx_privacy_logs_action ON privacy_logs(action);
