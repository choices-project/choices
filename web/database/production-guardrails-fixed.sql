-- Production Guardrails for Civics System (Fixed for Existing Data)
-- This version handles existing data with proper source and external_id assignment
...
-- 2. Add new columns to representatives table (without constraints first)
ALTER TABLE public.civics_representatives 
ADD COLUMN IF NOT EXISTS person_id UUID,
ADD COLUMN IF NOT EXISTS source TEXT,
ADD COLUMN IF NOT EXISTS external_id TEXT,
ADD COLUMN IF NOT EXISTS data_origin TEXT DEFAULT 'api' CHECK (data_origin IN ('manual', 'api')),
ADD COLUMN IF NOT EXISTS valid_from TIMESTAMPTZ DEFAULT NOW(),
ADD COLUMN IF NOT EXISTS valid_to TIMESTAMPTZ DEFAULT 'infinity'::timestamptz;

-- 3. Update existing data with proper source and external_id
-- Federal representatives (from GovTrack)
UPDATE public.civics_representatives 
SET 
  source = 'govtrack_api',
  external_id = COALESCE(
    raw_payload->'person'->>'bioguideid',
    raw_payload->'person'->>'osid',
    id::text
  )
WHERE level = 'federal' 
  AND (source IS NULL OR source = 'unknown');

-- State representatives (from OpenStates)
UPDATE public.civics_representatives 
SET 
...
-- Local representatives (manual verification)
UPDATE public.civics_representatives 
SET 
  source = CASE 
    WHEN jurisdiction = 'San Francisco, CA' THEN 'manual_verification_sf'
    WHEN jurisdiction = 'Los Angeles, CA' THEN 'manual_verification_la'
    ELSE 'manual_verification'
  END,
  external_id = LOWER(REGEXP_REPLACE(name, '[^a-zA-Z0-9]', '_', 'g')),
  data_origin = 'manual'
WHERE level = 'local' 
  AND (source IS NULL OR source = 'unknown');

-- 4. Handle any remaining duplicates by making external_id unique
UPDATE public.civics_representatives 
SET external_id = external_id || '_' || id::text
WHERE id IN (
  SELECT id FROM (
    SELECT id, ROW_NUMBER() OVER (PARTITION BY source, external_id ORDER BY created_at) as rn
    FROM public.civics_representatives
  ) t WHERE rn > 1
);

-- 5. Now add the unique constraint
...
-- 6. Add foreign key constraint for person_id
ALTER TABLE public.civics_representatives 
ADD CONSTRAINT fk_representatives_person_id 
FOREIGN KEY (person_id) REFERENCES public.civics_person_xref(person_id);

-- 7. Source Precedence Configuration
...
DROP POLICY IF EXISTS "Public read access for representatives" ON public.civics_representatives;
DROP POLICY IF EXISTS "Public read access for contact info" ON public.civics_contact_info;
DROP POLICY IF EXISTS "Public read access for social media" ON public.civics_social_engagement;
...
-- 11. Public Views for Client Access
...
SELECT 
  ...
  -- Source attribution
  CASE 
    WHEN r.source = 'govtrack_api' THEN 'GovTrack.us'
    WHEN r.source = 'openstates_api' THEN 'OpenStates.org'
    WHEN r.source = 'propublica_api' THEN 'ProPublica'
    WHEN r.source = 'google_civic_api' THEN 'Google Civic Information API'
    WHEN r.source = 'manual_verification_sf' THEN 'San Francisco City Clerk'
    WHEN r.source = 'manual_verification_la' THEN 'Los Angeles City Clerk'
    ELSE 'Unknown Source'
  END as source_attribution,
  CASE 
    WHEN r.source = 'govtrack_api' THEN 'https://www.govtrack.us'
    WHEN r.source = 'openstates_api' THEN 'https://openstates.org'
    WHEN r.source = 'propublica_api' THEN 'https://projects.propublica.org/api-docs/congress-api'
    WHEN r.source = 'google_civic_api' THEN 'https://developers.google.com/civic-information'
    ELSE NULL
  END as source_url
FROM public.civics_representatives r
WHERE r.valid_to = 'infinity'::timestamptz;

-- 12. Data Integrity Functions
...
-- 13. Minimal FEC Integration Table
...
-- 14. Minimal Voting Records Table
...
-- 16. Update trigger for all tables
