...
export class GeographicElectoralFeed {
  ...
  async generateElectoralFeed(
    ...
    try {
      devLog('Generating electoral feed', { userId, locationInput });
      ...
      devLog('Electoral feed generated successfully', {
        userId,
      ...
      devLog('Failed to generate electoral feed', { userId, error });
  ...
  private async getCurrentOfficials(location: UserLocation): Promise<{
    ...
    try {
      ...
      devLog('Failed to get current officials', { error });
    }
  ...
  private async getUpcomingElections(location: UserLocation): Promise<ElectoralRace[]> {
    ...
    try {
      devLog('Getting upcoming elections for location', { location });
      ...
      devLog('Failed to get upcoming elections', { error });
    }
  ...
  private async getActiveRaces(location: UserLocation): Promise<ElectoralRace[]> {
    try {
      devLog('Getting active races for location', { location });
      ...
      const enrichedRaces = await Promise.all(activeRaces.map(async (race) => {
        try {
          ...
          devLog('Failed to enrich race with campaign data', { raceId: race.raceId, error });
        }
      ...
      devLog('Failed to get active races', { error });
      return [];
    }
  }
  ...
  private async identifyKeyIssues(
    ...
    try {
      devLog('Identifying key issues for jurisdiction', { location });
      ...
      devLog('Failed to identify key issues', { error });
      return [];
    }
  }
