...
export class OptimizedSupabaseClient {
  ...
  async executeWithMonitoring<T>(
    ...
    try {
      ...
      const duration = performance.now() - start
      logger.error(`Query failed: ${operationName} after ${duration.toFixed(2)}ms`, error)
      throw error
    }
  }
...
export class OptimizedQueries {
   ...
   */
  async getCandidatesByState(state: string, limit: number = 100) {
    return this.client
      .from('candidates')
   ...
   */
  async getContributionsByCandidate(candidateId: number, limit: number = 100) {
    return this.client
      .from('contributions')
   ...
   */
  async getVotingRecordsByRepresentative(representativeId: number, limit: number = 100) {
    return this.client
      .from('voting_records')
   ...
   */
  async searchCandidates(query: string, limit: number = 50) {
    return this.client
      .from('candidates')
