...
// Poll with votes type for optimization queries
type PollWithVotes = Poll & {
  votes: Vote[]
  voteCount: number
}

// Vote with user info type for optimization queries
type VoteWithUserInfo = Vote & {
  user: {
    id: string
    username: string
    display_name?: string
...
export class QueryOptimizer {
  ...
  async getUserProfile(userId: string, useCache = true): Promise<MinimalUserProfile | null> {
    ...
    const queryPattern = 'SELECT user_profiles WHERE user_id = ?'
          ...
          if (useCache) {
            ...
            if (isFeatureEnabled('FEATURE_DB_OPTIMIZATION_SUITE')) {
              const { smartCache } = await import('@/lib/database/smart-cache')
              const cached = await smartCache.get<MinimalUserProfile>(cacheKey, queryPattern, DatabaseSchemas.UserProfile)
              if (cached) {
  ...
  async getPolls(options: {
    ...
    const pollsResponse: PollsResponse = {
      polls: validatedPolls.map(poll => {
        ...
        if (poll.expires_at) {
          return Object.assign({}, basePoll, { expires_at: poll.expires_at }) as unknown as PollWithVotes
        }
        return basePoll as unknown as PollWithVotes
      }),
  ...
  async getVotesForPoll(pollId: string, options: {
    ...
    if (groupByChoice && result.data && Array.isArray(result.data) && !result.error) {
      // Group votes by option_id for analytics
      const grouped = (result.data as unknown as Vote[]).reduce((acc, vote) => {
        const choice = vote.option_id || 'unknown'
        if (!acc[choice]) {
          acc[choice] = []
        }
  ...
  const processTimeSeries = (data: Array<{ created_at: string }>, dateField: string) => {
    ...
    return {
      period,
      summary: `Users: ${totalUsers}, Polls: ${totalPolls}, Votes: ${totalVotes}, Active: ${userGrowth.length}, New Polls: ${pollActivity.length}, New Votes: ${voteActivity.length}`,
      trends: {
