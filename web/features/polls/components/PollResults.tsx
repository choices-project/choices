...
export default function OptimizedPollResults({
  ...
  const pollStatusDisplay = useMemo(() => {
    ...
    const statusConfig = {
      ended: { label: t('polls.view.results.status.ended'), color: 'text-red-600 dark:text-red-400', bgColor: 'bg-red-50 dark:bg-red-900/30' },
      active: { label: t('polls.view.results.status.active'), color: 'text-green-600 dark:text-green-400', bgColor: 'bg-green-50 dark:bg-green-900/30' },
      ongoing: { label: t('polls.view.results.status.ongoing'), color: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-50 dark:bg-blue-900/30' }
    }
  ...
  const handleRefresh = useCallback(async () => {
    ScreenReaderSupport.announce(tRef.current('polls.view.results.refreshing'), 'polite');
    await refetch();
    loadCacheStats();
  ...
  useEffect(() => {
    ...
    const summary = tRef.current('polls.view.results.summary', {
      totalVotes,
  ...
  if (error) {
    return (
      <EnhancedErrorDisplay
        title={tRef.current('polls.view.results.error.title') || 'Error loading poll results'}
        message={error}
        ...
        primaryAction={{
          label: tRef.current('polls.view.results.error.retry') || 'Try Again',
  ...
  if (!results) {
    return (
      <EnhancedEmptyState
        ...
        title={tRef.current('polls.view.results.empty.title') || 'No poll results available'}
        description={tRef.current('polls.view.results.empty.description') || 'Poll results are not yet available. Results may be processing or the poll may not have received any votes yet.'}
    ...
    <div className="space-y-6">
      ...
      {showPerformanceMetrics && performanceMetrics && (
        <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          <h4 className="text-sm font-medium text-blue-900 dark:text-blue-200 mb-2">{t('polls.view.results.performance.title')}</h4>
