...
/**
 * Deploy Automated Polls Tables
 * 
 * Creates the missing database tables for the automated polls feature
 */

const { createClient } = require('@supabase/supabase-js');
require('dotenv').config({ path: './web/.env.local' });

async function deployAutomatedPollsTables() {
...
`;
  ...
  try {
    console.log('📋 Creating automated polls tables...');
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        if (statement.trim()) {
          try {
            ...
            console.log(`⚠️  Statement ${i + 1} failed (may already exist): ${e.message}`);
          }
        }
      }
    } else {
    ...
    console.log('\n🎉 Automated polls tables deployment completed!');
    console.log('\n📋 Next Steps:');
