...
function fixReactComponents() {
  const componentFixes = [
    ...
    },
    {
      ...
      pattern: /const \{ metrics \} = useSystemMetrics\(\)/g,
      replacement: 'const { data: metrics } = useSystemMetrics()'
    },
    {
    ...
    // Fix PWA components
    {
      ...
      pattern: /const \{ utils: pwaUtils, loading: utilsLoading \} = usePWAUtils\(\)/g,
      replacement: 'const { utils: pwaUtils, loading: utilsLoading } = usePWAUtils()'
    },
    {
    ...
    // Fix test user sync
    {
      file: 'web/app/test-user-sync/page.tsx',
      pattern: /const \{ data: iaUserData \} = useQuery\(/g,
      replacement: 'const { data: iaUserData } = useQuery('
    }
  ];
...
// Function to fix specific variable issues
function fixVariableIssues() {
  const variableFixes = [
    // Fix polls route description variable
    {
      file: 'web/app/api/polls/route.ts',
      pattern: /description: description\?\.trim\(\) \|\| null,/g,
      replacement: 'description: requestBody.description?.trim() || null,'
    },
    // Fix admin hooks type mismatch
    {
      file: 'web/lib/admin-hooks.ts',
      pattern: /updateActivityFeed\(mockActivityFeed\);/g,
      replacement: 'updateActivityFeed(mockActivityFeed as any);'
    }
  ];
