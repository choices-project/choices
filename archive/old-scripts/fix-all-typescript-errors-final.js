...
function fixAllErrors() {
  const fixes = [
    // Fix admin breaking news route - GET method
    {
      file: 'web/app/api/admin/breaking-news/[id]/poll-context/route.ts',
      search: '    // Check admin permissions - RESTRICTED TO OWNER ONLY\n    const {  } = await supabase',
      replace: '    // Check admin permissions - RESTRICTED TO OWNER ONLY\n    const { data: userProfile, error: profileError } = await supabase'
    },
    {
      file: 'web/app/api/admin/breaking-news/[id]/poll-context/route.ts',
      search: '    // Get existing poll context\n    const {  } = await supabase',
      replace: '    // Get existing poll context\n    const { data: contextData, error: contextError } = await supabase'
    },
...
function fixVariableIssues() {
  const variableFixes = [
    // Fix register route userPassword variable
    {
      file: 'web/app/api/auth/register/route.ts',
    ...
    try {
      if (fs.existsSync(fix.file)) {
        ...
        if (content.includes(fix.search)) {
          const newContent = content.replace(fix.search, fix.replace);
          fs.writeFileSync(fix.file, newContent, 'utf8');
          console.log(`✅ Fixed variable: ${fix.file}`);
        } else {
          console.log(`⚠️  Variable pattern not found: ${fix.file}`);
        }
      }
    } catch (error) {
      console.log(`❌ Error fixing variable in ${fix.file}:`, error.message);
    }
  });
}
