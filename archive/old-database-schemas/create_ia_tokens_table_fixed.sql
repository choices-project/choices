-- Create ia_tokens table for IA/PO architecture (FIXED VERSION)
...
DO $$
BEGIN
    -- Add foreign key to po_polls if it exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'po_polls') THEN
...
-- Insert sample data for testing (only if po_polls table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'po_polls') THEN
