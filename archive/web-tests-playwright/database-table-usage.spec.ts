/**
 * Database Table Usage E2E Test
 * 
 * Comprehensive test to track which database tables are actually used
 * across all features in the Choices ecosystem
 * 
...
test.describe('Database Table Usage - Complete Feature Coverage', () => {
  test('should track all tables used during complete user lifecycle', async ({ page }) => {
    // Reset database tracking
    DatabaseTracker.reset();
    
    console.log('🚀 Starting Database Table Usage E2E Test');
    ...
    // 1.2 Fill registration form using proper test IDs
    await page.fill(`[data-testid="${T.login.email}"]`, 'test@example.com');
    await page.fill(`[data-testid="${T.login.password}"]`, 'password123');
    await page.click(`[data-testid="${T.login.submit}"]`);
    
    // 1.3 Wait for authentication to complete
    await page.waitForLoadState('networkidle');
    
    // 1.4 WebAuthn biometric setup (if available)
    const biometricButton = page.locator(`[data-testid="${T.biometricSetupButton}"]`);
    if (await biometricButton.isVisible()) {
      await biometricButton.click();
      await page.waitForSelector(`[data-testid="${T.webauthn.biometricPrompt}"]`, { timeout: 10000 });
    }
    ...
    // 2.2 Profile setup
    await page.fill(`[data-testid="${T.onboarding.usernameInput}"]`, 'Test User');
    await page.fill(`[data-testid="${T.onboarding.bioTextarea}"]`, 'Test bio for comprehensive testing');
    await page.selectOption('[data-testid="participation-style"]', 'participant');
    
    // 2.3 Demographics survey
    await page.selectOption(`[data-testid="${T.onboarding.ageInput}"]`, '25-34');
    await page.selectOption('[data-testid="education"]', 'bachelor');
    await page.selectOption('[data-testid="income"]', '50000-75000');
    
    // 2.4 Location input
    await page.fill(`[data-testid="${T.onboarding.locationInput}"]`, 'Washington, DC');
    ...
    await page.click(`[data-testid="${T.onboarding.notificationsCheckbox}"]`);
    await page.click('[data-testid="consent-location"]');
    await page.click(`[data-testid="${T.onboarding.dataSharingCheckbox}"]`);
    
    // 2.8 Complete onboarding
    await page.click(`[data-testid="${T.completeOnboarding}"]`);
    ...
    await page.waitForLoadState('networkidle');
    await page.fill(`[data-testid="${T.pollCreate.title}"]`, 'Test Poll: What should we prioritize?');
    await page.fill(`[data-testid="${T.pollCreate.description}"]`, 'This is a comprehensive test poll for E2E testing');
    
    // 4.2 Add poll options
    await page.fill(`[data-testid="${T.pollCreate.optionInput(1)}"]`, 'Climate Action');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(2)}"]`, 'Economic Reform');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(3)}"]`, 'Healthcare Access');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(4)}"]`, 'Education Investment');
    
    // 4.3 Set voting method
    await page.selectOption(`[data-testid="${T.pollCreate.votingMethod}"]`, 'single_choice');
    
    // 4.4 Set privacy level
    await page.selectOption(`[data-testid="${T.pollCreate.privacyLevel}"]`, 'public');
    ...
    await page.click(`[data-testid="${T.voteButton}"]`);
    await page.waitForSelector('[data-testid="vote-submitted"]');
    
    // 4.9 View poll results
    await page.click(`[data-testid="${T.poll.results}"]`);
    await page.waitForSelector(`[data-testid="${T.poll.results}"]`);
    ...
    // 8.1 Install PWA
    await page.click(`[data-testid="${T.pwa.installButton}"]`);
    await page.waitForSelector(`[data-testid="${T.pwa.installedIndicator}"]`);
    
    // 8.2 Test offline functionality
    await page.context().setOffline(true);
    await page.goto('/polls');
    await page.waitForSelector(`[data-testid="${T.pwa.offlineIndicator}"]`);
    
    // 8.3 Test push notifications
    await page.context().setOffline(false);
    await page.click(`[data-testid="${T.pwa.requestPermissionButton}"]`);
    await page.waitForSelector(`[data-testid="${T.pwa.notificationPermission}"]`);
    
    // 8.4 Test background sync
    await page.click(`[data-testid="${T.pwa.syncButton}"]`);
    ...
    await page.waitForLoadState('networkidle');
    await page.fill(`[data-testid="${T.login.email}"]`, 'admin@example.com');
    await page.fill(`[data-testid="${T.login.password}"]`, 'admin123');
    await page.click(`[data-testid="${T.login.submit}"]`);
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForSelector(`[data-testid="${T.admin.userList}"]`);
    
    // 9.3 View poll moderation
    await page.goto('/admin/polls');
    await page.waitForLoadState('networkidle');
    await page.waitForSelector(`[data-testid="${T.admin.pollList}"]`);
    ...
    await page.click(`[data-testid="${T.submitButton}"]`);
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForSelector(`[data-testid="${T.systemStatus}"]`);
    ...
    await page.selectOption(`[data-testid="${T.pollCreate.votingMethod}"]`, 'single_choice');
    await page.fill(`[data-testid="${T.pollCreate.title}"]`, 'Single Choice Voting Test');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(1)}"]`, 'Option A');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(2)}"]`, 'Option B');
    await page.click(`[data-testid="${T.pollCreate.submit}"]`);
    await page.waitForSelector('[data-testid="poll-created"]');
    ...
    await page.goto('/polls/create');
    await page.waitForLoadState('networkidle');
    await page.selectOption(`[data-testid="${T.pollCreate.votingMethod}"]`, 'multiple_choice');
    await page.fill(`[data-testid="${T.pollCreate.title}"]`, 'Multiple Choice Test');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(1)}"]`, 'Option 1');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(2)}"]`, 'Option 2');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(3)}"]`, 'Option 3');
    await page.click(`[data-testid="${T.pollCreate.submit}"]`);
    await page.waitForSelector('[data-testid="poll-created"]');
    ...
    await page.goto('/polls/create');
    await page.waitForLoadState('networkidle');
    await page.selectOption(`[data-testid="${T.pollCreate.votingMethod}"]`, 'ranked_choice');
    await page.fill(`[data-testid="${T.pollCreate.title}"]`, 'Ranked Choice Test');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(1)}"]`, 'First Choice');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(2)}"]`, 'Second Choice');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(3)}"]`, 'Third Choice');
    await page.click(`[data-testid="${T.pollCreate.submit}"]`);
    await page.waitForSelector('[data-testid="poll-created"]');
    ...
    await page.goto('/polls/create');
    await page.waitForLoadState('networkidle');
    await page.selectOption(`[data-testid="${T.pollCreate.votingMethod}"]`, 'approval');
    await page.fill(`[data-testid="${T.pollCreate.title}"]`, 'Approval Voting Test');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(1)}"]`, 'Approve A');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(2)}"]`, 'Approve B');
    await page.fill(`[data-testid="${T.pollCreate.optionInput(3)}"]`, 'Approve C');
    await page.click(`[data-testid="${T.pollCreate.submit}"]`);
    await page.waitForSelector('[data-testid="poll-created"]');
    ...
    const analysis = TableUsageAnalyzer.analyzeUsage(report);
    ...
    console.log(`📊 Tables Used: ${usedTables.length}/${allTables.length} (${((usedTables.length / allTables.length) * 100).toFixed(1)}%)`);
    console.log(`🗑️ Unused Tables: ${analysis.unusedTables.length} tables`);
    console.log(`⚠️ Data Integrity Issues: ${analysis.dataIntegrityIssues.length} tables`);
  });
});
