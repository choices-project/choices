...
test.describe('Authenticated User Journey Test', () => {
  test('should track database usage through authenticated user journey', async ({ page }) => {
    ...
    try {
      ...
      DatabaseTracker.trackQuery('webauthn_credentials', 'select', 'user_login');
    ...
    try {
      ...
      if (profileResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('user_notification_preferences', 'select', 'profile_api_authenticated');
        DatabaseTracker.trackQuery('user_preferences', 'select', 'profile_api_authenticated');
      } else {
    ...
    try {
      ...
      if (dashboardResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('polls', 'select', 'dashboard_api_authenticated');
        DatabaseTracker.trackQuery('votes', 'select', 'dashboard_api_authenticated');
    ...
    try {
      ...
      if (analyticsResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('analytics_contributions', 'select', 'analytics_api_authenticated');
        DatabaseTracker.trackQuery('analytics_demographics', 'select', 'analytics_api_authenticated');
      } else {
    ...
    try {
      ...
      if (createPollResponse.status() === 200 || createPollResponse.status() === 201) {
        ...
        DatabaseTracker.trackQuery('polls', 'insert', 'poll_creation_authenticated');
        DatabaseTracker.trackQuery('poll_options', 'insert', 'poll_creation_authenticated');
        DatabaseTracker.trackQuery('poll_analytics', 'insert', 'poll_creation_authenticated');
      } else {
    ...
    try {
      ...
      if (voteResponse.status() === 200 || voteResponse.status() === 201) {
        ...
        DatabaseTracker.trackQuery('votes', 'insert', 'poll_voting_authenticated');
        DatabaseTracker.trackQuery('poll_analytics', 'update', 'poll_voting_authenticated');
      } else {
    ...
    try {
      ...
      if (profileUpdateResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('user_profiles', 'update', 'profile_update_authenticated');
        DatabaseTracker.trackQuery('user_preferences', 'update', 'profile_update_authenticated');
      } else {
    ...
    try {
      ...
      if (adminDashboardResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('user_profiles', 'select', 'admin_dashboard');
        DatabaseTracker.trackQuery('polls', 'select', 'admin_dashboard');
        DatabaseTracker.trackQuery('votes', 'select', 'admin_dashboard');
        DatabaseTracker.trackQuery('analytics_events', 'select', 'admin_dashboard');
        DatabaseTracker.trackQuery('location_consent_audit', 'select', 'admin_dashboard');
      } else {
    ...
    try {
      ...
      if (adminAnalyticsResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('trust_tier_analytics', 'select', 'admin_analytics');
        DatabaseTracker.trackQuery('analytics_user_engagement', 'select', 'admin_analytics');
        DatabaseTracker.trackQuery('analytics_contributions', 'select', 'admin_analytics');
    ...
    try {
      ...
      if (hashtagsResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('hashtags', 'select', 'hashtags_api_with_params');
        DatabaseTracker.trackQuery('hashtag_usage', 'select', 'hashtags_api_with_params');
        DatabaseTracker.trackQuery('hashtag_trends', 'select', 'hashtags_api_with_params');
      } else {
    ...
    try {
      ...
      if (trendingResponse.status() === 200) {
        ...
        DatabaseTracker.trackQuery('hashtag_trends', 'select', 'trending_api_authenticated');
        DatabaseTracker.trackQuery('hashtag_usage', 'select', 'trending_api_authenticated');
        DatabaseTracker.trackQuery('hashtags', 'select', 'trending_api_authenticated');
      } else {
    ...
    await DatabaseTracker.saveReport('authenticated-user-journey-test.json');
