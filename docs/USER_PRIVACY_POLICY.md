# Privacy Policy - Ranked Choice Democracy Platform

**Last Updated:** January 15, 2025  
**Effective Date:** January 15, 2025

## Your Privacy Matters

We believe in transparent, user-controlled data practices. This policy explains what data we collect, why we collect it, and how you can control it.

## What Data We Collect

### Essential Data (Required for Platform Function)
- **Account Information:** Email, username, password (encrypted)
- **Voting Records:** Your poll votes and preferences (anonymous)
- **Representative Data:** Your elected officials and districts

### Analytics Data (Optional - You Control This)
- **Usage Patterns:** Which pages you visit and how you navigate
- **Performance Data:** How fast pages load and any errors
- **Device Information:** Browser type and device for accessibility
- **Geographic Location:** Your state/region for representative accuracy

### Privacy Data (For Your Rights)
- **Consent Records:** What you've agreed to share
- **Data Requests:** When you ask for your data or deletion
- **Audit Logs:** Security and access records

## Why We Collect This Data

### For Democracy & Election Integrity
- **Prevent Bot Manipulation:** Stop automated attacks on polls
- **Ensure Fair Representation:** Verify geographic distribution
- **Maintain Trust:** Protect the integrity of democratic processes

### For Better User Experience
- **Improve Navigation:** Make the platform easier to use
- **Fix Performance Issues:** Speed up slow pages
- **Enhance Accessibility:** Support users with different needs

### For Legal Compliance
- **Meet Regulations:** Comply with privacy laws
- **Protect Your Rights:** Enable data access and deletion
- **Maintain Security:** Monitor for threats and attacks

## Your Control & Rights

### Granular Consent
You can choose exactly what data to share:
- ✅ **Essential Data:** Required for platform function
- 🔒 **Analytics Data:** Optional - improve your experience
- 📍 **Location Data:** Optional - show relevant representatives
- 📊 **Usage Data:** Optional - help us improve the platform

### Your Rights (GDPR/CCPA Compliant)
- **Access:** See all data we have about you
- **Correction:** Fix any inaccurate information
- **Deletion:** Request complete data removal
- **Portability:** Download your data in standard formats
- **Restriction:** Limit how we use your data
- **Objection:** Opt out of certain data processing

### How to Exercise Your Rights
1. **Account Settings:** Control most preferences in your account
2. **Email:** Contact privacy@rankedchoicedemocracy.org
3. **Online Portal:** Use our data rights portal
4. **Response Time:** We respond within 30 days

## Data Protection Measures

### Security
- **Encryption:** All data encrypted in transit and at rest
- **Access Controls:** Strict limits on who can see your data
- **Regular Audits:** Security reviews and updates
- **Incident Response:** Rapid response to any security issues

### Data Minimization
- **Only Necessary:** We collect only what we need
- **Purpose Limitation:** Data used only for stated purposes
- **Retention Limits:** Automatic deletion after specified periods
- **No Cross-Site Tracking:** We don't track you across other websites

### Your Privacy Controls
- **Opt-Out Anytime:** Withdraw consent at any time
- **Data Export:** Download your data in standard formats
- **Account Deletion:** Complete account and data removal
- **Anonymous Usage:** Use the platform without personal data

## Data Retention

### How Long We Keep Data
- **Analytics Data:** 30 days, then automatically deleted
- **User Profiles:** 1 year, then anonymized
- **Audit Logs:** 90 days, then deleted
- **Consent Records:** 7 years (legal requirement)
- **Voting Records:** Anonymized immediately, no personal connection

### Automatic Deletion
- **Scheduled Cleanup:** Regular automatic data purging
- **Retention Policies:** Clear rules for each data type
- **User Requests:** Immediate deletion upon request
- **Legal Requirements:** Some data retained for compliance

## Third-Party Sharing

### We Don't Sell Your Data
- **No Data Sales:** We never sell personal information
- **No Advertising:** We don't use data for advertising
- **No Cross-Site Tracking:** We don't track you elsewhere
- **Limited Sharing:** Only with your explicit consent

### Service Providers
- **Hosting:** Secure cloud infrastructure
- **Analytics:** Privacy-focused analytics tools
- **Security:** Security monitoring services
- **Legal Compliance:** Data protection consultants

## International Users

### Data Transfers
- **Adequate Protection:** Data transferred only to countries with adequate protection
- **Safeguards:** Technical and contractual protections
- **Your Rights:** Same rights regardless of location
- **Local Laws:** Compliance with your local privacy laws

### Geographic Restrictions
- **US Users:** CCPA compliance
- **EU Users:** GDPR compliance
- **Other Regions:** Local privacy law compliance
- **Universal Rights:** Core privacy rights for all users

## Children's Privacy

### Age Restrictions
- **Minimum Age:** 13 years old to use the platform
- **Parental Consent:** Required for users under 16
- **No Child Data:** We don't knowingly collect data from children
- **Immediate Deletion:** Child data deleted upon discovery

### Protection Measures
- **Age Verification:** Basic age verification systems
- **Parental Controls:** Parental oversight options
- **Educational Content:** Age-appropriate civic education
- **Safe Environment:** Child-safe platform features

## Changes to This Policy

### Notification
- **Email Notice:** We'll email you about significant changes
- **In-App Notice:** Platform notifications for updates
- **Version History:** Track all policy changes
- **Re-Consent:** New consent for material changes

### Your Options
- **Accept Changes:** Continue using the platform
- **Modify Consent:** Adjust your privacy preferences
- **Withdraw Consent:** Opt out of new data collection
- **Account Deletion:** Remove your account and data

## Contact Us

### Privacy Questions
- **Email:** privacy@rankedchoicedemocracy.org
- **Data Rights:** rights@rankedchoicedemocracy.org
- **Security Issues:** security@rankedchoicedemocracy.org
- **General Questions:** support@rankedchoicedemocracy.org

### Response Times
- **General Inquiries:** 48 hours
- **Data Rights Requests:** 30 days maximum
- **Security Issues:** 24 hours
- **Urgent Matters:** Immediate response

## Your Trust Matters

We're committed to protecting your privacy while enabling democratic participation. This platform only works if users trust it with their data and their votes.

### Our Commitments
- **Transparency:** Clear, honest communication about data use
- **Control:** You decide what data to share
- **Security:** Strong protection for your information
- **Rights:** Easy access to your privacy rights
- **Integrity:** Data used only for stated, legitimate purposes

### Questions or Concerns?
We're here to help. Contact us anytime with privacy questions or concerns. Your privacy and trust are essential to our mission of improving democracy.

## Technical Implementation

### Database Security
Your privacy data is protected by multiple layers of security:

- **Row Level Security (RLS)**: You can only access your own data
- **Encryption**: All data encrypted in transit and at rest
- **Access Controls**: Strict authentication required
- **Audit Logging**: All data access is tracked and logged

### Data Storage Schema
We store your privacy preferences in secure, purpose-built database tables:

#### **Your Privacy Settings**
- **Consent Records**: What you've agreed to share
- **Data Requests**: When you ask for your data or deletion
- **Audit Logs**: Security and access records
- **Analytics Events**: Optional usage data (if you consent)

#### **Data Categories We Track**
- **Essential Data**: Account info, voting records, representatives (required)
- **Analytics Data**: Usage patterns, performance data (optional)
- **Location Data**: State/region for representative accuracy (optional)
- **Demographic Data**: Age, education for relevant polls (optional)

### Your Data Rights Implementation
All your rights are technically implemented in our database:

- **Right of Access**: Complete data export functionality
- **Right to Rectification**: Profile editing capabilities
- **Right to Erasure**: Account deletion with data removal
- **Right to Portability**: JSON/CSV data export
- **Right to Restrict Processing**: Granular consent controls
- **Right to Object**: Opt-out mechanisms for all data types

### Privacy Controls Available
You have full control through our user interface:

- **Privacy Settings Page**: `/account/privacy`
- **Data Export**: `/account/export`
- **Account Deletion**: `/account/delete`
- **Consent Management**: Real-time preference updates
- **Data Retention**: Choose how long we keep your data

### Security Measures
- **Encryption**: AES-256 encryption for all data
- **Authentication**: Multi-factor authentication support
- **Access Logging**: All data access is logged and monitored
- **Regular Audits**: Security reviews and updates
- **Incident Response**: Rapid response to any security issues

---

**This privacy policy is designed to be clear, comprehensive, and user-friendly. We believe in transparent data practices that put you in control.**
