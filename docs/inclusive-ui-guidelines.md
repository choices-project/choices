...
| Data Visualisations | Provide textual summaries, keyboard support, captions. Use `AccessibleResultsChart` and `AnalyticsSummaryTable` patterns; wrap dashboards in named regions with live status updates. | [`components/accessible/AccessibleResultsChart.tsx`](../web/components/accessible/AccessibleResultsChart.tsx), [`features/analytics/components/AnalyticsSummaryTable.tsx`](../web/features/analytics/components/AnalyticsSummaryTable.tsx), [`features/analytics/components/EnhancedAnalyticsDashboard.tsx`](../web/features/analytics/components/EnhancedAnalyticsDashboard.tsx) |
...
| Feed Providers & Notifications | Translate loading/refresh/error copy (including harness flows), announce toast state changes through `useNotificationActions`, and re-use `Intl.NumberFormat` for counts. | `features/feeds/components/providers/FeedDataProvider.tsx`, `features/feeds/components/InfiniteScroll.tsx` |
| Personal Dashboard & Analytics | All dashboard panels (overview, trending, representatives, settings) pull copy from `dashboard.personal.*`, format numbers/dates via locale-aware helpers, and provide aria labels for quick actions. Harness render must match production semantics. | `features/dashboard/components/PersonalDashboard.tsx` |
| Widget Dashboards & Layout Editors | Provide keyboard affordances for move/resize, keep edit mode instructions localised, surface in-product keyboard help, and ensure `ScreenReaderSupport` announces state changes. Automated spec (`widget-dashboard-keyboard.spec.ts`) must stay green. | [`features/analytics/components/widgets/WidgetRenderer.tsx`](../web/features/analytics/components/widgets/WidgetRenderer.tsx), [`features/analytics/components/widgets/WidgetDashboard.tsx`](../web/features/analytics/components/widgets/WidgetDashboard.tsx), [`tests/e2e/specs/widget-dashboard-keyboard.spec.ts`](../web/tests/e2e/specs/widget-dashboard-keyboard.spec.ts), [`store-roadmaps/widget-store-checklist`](../scratch/gpt5-codex/store-roadmaps/widget-store-checklist.md) |
...
- **Announcements:** Broadcast significant state changes (`aria-live`) using existing helpers (e.g., vote submission, PWA updates). Layout editors must announce entry/exit of edit/move/resize modes and positional updates.
