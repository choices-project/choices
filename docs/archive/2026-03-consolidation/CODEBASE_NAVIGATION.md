...
- `lib/stores/` - Zustand stores
  ...
  - `userStore.ts` - Auth state, user session, logout cascade
  - `contactStore.ts` - Contact system state
  - `hashtagStore.ts` - Hashtag management state
  - `votingStore.ts` - Voting session state
  - `feedsStore.ts` - Feed content and filtering
...
- `components/shared/AnimatedCard.tsx` - Framer Motion animated card wrapper
- `components/shared/AnimatedVoteBar.tsx` - Animated vote result bars
- `components/shared/PageTransition.tsx` - Route transition animations
- `components/shared/BottomSheet.tsx` - Mobile bottom sheet component
- `components/shared/ScrollRestoration.tsx` - Browser scroll restoration
...
| Animations | `@/lib/animations` | `fadeUp`, `staggerContainer`, `springScale` |
| Design tokens | `globals.css` variables | `bg-card`, `text-foreground`, `border-border` |
