...
5. State refreshers (`state:refresh`, `state:sync:contacts`, `state:sync:social`, `state:sync:photos`, `state:sync:committees`, `state:sync:activity`, `state:sync:data-sources`, `state:sync:google-civic`, `state:sync:google-elections`, `state:sync:divisions`) remain available with `--dry-run` until the SQL-first flow covers every table.
...
| `state:sync:google-elections` | Pull upcoming elections from Google Civic (`civic_elections`) | `civic_elections` | Supports `--dry-run`; runs nationwide (filter downstream via division) |
| `state:sync:divisions` | Rebuild representative → OCD division mappings | `representative_divisions` | Calls `refresh_divisions_from_openstates()` |
