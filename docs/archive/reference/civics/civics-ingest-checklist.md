...
- [ ] `npm run state:sync:google-elections -- --dry-run` (preview election sync) before running live.
- [ ] `npm run state:sync:google-civic -- --states=CA --dry-run` (supplement contacts/social/photos) before running live.
- [ ] `npm run state:sync:divisions -- --dry-run` (confirm OCD division mappings) before running live.
