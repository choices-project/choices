...
- **`global-error.tsx`** root boundary + **5** route-level `error.tsx` + **2** `loading.tsx` files under `web/app/` (counts checked by `npm run verify:architecture-boundaries` / `verify:docs`)
