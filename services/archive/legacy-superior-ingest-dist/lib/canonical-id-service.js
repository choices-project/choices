...
export class CanonicalIdService {
     ...
     */
    generateCanonicalId(entityType, primaryData) {
        switch (entityType) {
            case 'person':
     ...
     */
    async upsertCrosswalkEntry(entityType, canonicalId, source, sourceId, attrs = {}) {
     ...
     */
    async getCrosswalkEntries(canonicalId) {
        const { data, error } = await this.supabase
            .from('id_crosswalk')
            .select('*')
     ...
     */
    async getCanonicalIdMapping(canonicalId) {
        ...
        return {
            canonical_id: canonicalId,
            sources,
            entity_type: entries[0]?.entity_type || 'representative'
        };
    }
    /**
     * Find canonical ID by source ID
     */
    async findCanonicalIdBySource(source, sourceId) {
        const { data, error } = await this.supabase
            .from('id_crosswalk')
            .select('canonical_id')
            .eq('source', source)
    ...
    async resolveEntity(entityType, sourceData) {
        ...
        if (!canonicalId) {
            ...
            const priorityOrder = ['congress-gov', 'fec', 'open-states', 'opensecrets', 'govtrack', 'google-civic'];
        ...
        for (const { source, data, sourceId } of sourceData) {
            const entry = await this.upsertCrosswalkEntry(entityType, canonicalId, source, sourceId, { quality_score: this.calculateDataQuality(data) });
     ...
     */
    async getEntitySources(canonicalId) {
        const entries = await this.getCrosswalkEntries(canonicalId);
...
//# sourceMappingURL=canonical-id-service.js.map