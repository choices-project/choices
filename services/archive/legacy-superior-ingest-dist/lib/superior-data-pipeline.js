...
import { CanonicalIdService } from './canonical-id-service.js';
import { CurrentElectorateVerifier } from './current-electorate-verifier.js';
import OpenStatesIntegration from './openstates-integration.js';
...
export class SuperiorDataPipeline {
    ...
    constructor(config) {
        this.config = config;
        this.supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { persistSession: false } });
     ...
     */
    async processFederalRepresentatives(limit = 100) {
     ...
     */
    async processStateRepresentatives(limit = 100, state) {
                ...
                if (this.config.enableOpenStatesPeople) {
                    try {
                        ...
                        if (openStatesData && openStatesData.length > 0) {
                            secondaryData = {
                                secondaryData: {
                                    openStatesPerson: openStatesData.find(person => person.name && rep.name &&
    ...
    extractContacts(primaryData, secondaryData) {
        ...
        if (secondaryData?.secondaryData?.openStatesPerson?.offices) {
            secondaryData.secondaryData.openStatesPerson.offices.forEach((office) => {
                if (office.voice) {
                    contacts.push({
                        type: 'phone',
                        value: office.voice,
    ...
    extractActivity(primaryData, secondaryData) {
        ...
        if (primaryData.congressGov?.sponsoredLegislation?.bills) {
            primaryData.congressGov.sponsoredLegislation.bills.slice(0, 3).forEach((bill) => {
                activity.push({
                    type: 'bill_sponsored',
        ...
        if (primaryData.googleCivic?.elections) {
            primaryData.googleCivic.elections.slice(0, 3).forEach((election) => {
                activity.push({
                    type: 'election',
        ...
        if (secondaryData?.openStatesPerson?.roles) {
            secondaryData.openStatesPerson.roles.forEach((role) => {
                if (role.committee) {
                    activity.push({
    ...
    extractSocialMedia(primaryData, secondaryData) {
        ...
        if (primaryData.googleCivic?.channels) {
            primaryData.googleCivic.channels.forEach((channel) => {
                socialMedia.push({
        ...
        if (secondaryData?.openStatesPerson?.social_media) {
            secondaryData.openStatesPerson.social_media.forEach((sm) => {
                socialMedia.push({
        ...
        if (secondaryData?.fecData?.candidate?.socialMedia) {
            secondaryData.fecData.candidate.socialMedia.forEach((sm) => {
                socialMedia.push({
                    platform: sm.platform,
                    handle: sm.handle,
                    url: sm.url,
    ...
    calculateComprehensiveDataQuality(primaryData, secondaryData, contacts, photos, activity) {
        ...
        if (primaryData.congressGov)
            primarySourceScore += 50; // Congress.gov is primary for federal
        if (primaryData.fec)
            primarySourceScore += 20; // FEC is important for federal
        if (primaryData.googleCivic)
            primarySourceScore += 15; // Google Civic for elections and voter info
        if (primaryData.wikipedia)
            primarySourceScore += 15; // Wikipedia for biographical info
        // Note: OpenStates API not applicable for federal representatives
        // Secondary source scoring
        if (secondaryData?.secondaryData) {
            secondarySourceScore += 50;
            if (secondaryData.secondaryData.openStatesPerson?.offices?.length > 0)
                secondarySourceScore += 20;
            if (secondaryData.secondaryData.openStatesPerson?.contact_details?.length > 0)
                secondarySourceScore += 15;
            if (secondaryData.secondaryData.openStatesPerson?.sources?.length > 0)
                secondarySourceScore += 10;
        }
    ...
    calculateDataCompleteness(contacts, photos, activity) {
        ...
        if (contacts.some(c => c.isPrimary))
            score += 10;
        if (photos.some(p => p.source === 'wikipedia'))
            score += 10;
            ...
            for (const enhancedRep of enhancedRepresentatives) {
                ...
                if (enhancedRep.canonicalId) {
                    ...
                    if (crosswalkData && crosswalkData.length > 0) {
                        ...
                        const sourceIds = crosswalkData.map((entry) => entry.source_id);
                        const sourceFields = crosswalkData.map((entry) => {
                            switch (entry.source) {
     ...
     */
    async populateEnhancedTables(representativeId, enhancedRep) {
        try {
    ...
    async getGoogleCivicData(rep) {
        ...
        try {
            ...
            const address = stateCapitals[rep.state] || `${rep.state}, USA`;
    ...
    async getOpenStatesApiData(rep) {
        ...
        try {
            ...
            if (rep.openstates_id) {
                ...
                const response = await fetch(`https://v3.openstates.org/people/${rep.openstates_id}`, {
                    method: 'GET',
                    headers: {
     ...
     */
    async getOpenStatesApiDataByJurisdiction(rep, apiKey) {
        ...
        const response = await fetch(`https://v3.openstates.org/people?jurisdiction=${rep.state}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
        ...
        const matchingLegislator = data.results?.find((legislator) => legislator.name === rep.name ||
     ...
     */
    extractOpenStatesSocialMedia(legislator) {
        const socialMedia = [];
     ...
     */
    mapOpenStatesPlatform(platform) {
        const platformMap = {
            'twitter': 'twitter',
            'facebook': 'facebook',
            'instagram': 'instagram',
     ...
     */
    generateSocialMediaUrl(platform, handle) {
        const urlMap = {
    ...
    async getWikipediaData(rep) {
        ...
        try {
            ...
            for (const nameFormat of nameFormats) {
                ...
                const response = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(nameFormat)}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
     ...
     */
    hasCurrentParty(openStatesPerson) {
        if (!openStatesPerson?.roles)
            return false;
        const currentDate = new Date();
        const currentRoles = openStatesPerson.roles.filter((role) => {
    ...
    async resolveCanonicalId(rep) {
        try {
            ...
            const result = await this.canonicalIdService.resolveEntity('person', sourceData);
     ...
     * Fetch federal representatives from Congress.gov API
     */
    async fetchFederalRepresentatives(limit) {
        try {
     ...
     * Fetch state representatives from OpenStates
     */
    async fetchStateRepresentatives(limit, state) {
        try {
            if (!this.config.enableOpenStatesPeople) {
            ...
            if (state) {
                ...
                const transformed = representatives.map((rep, index) => {
                ...
                for (const stateCode of states) {
                    if (allRepresentatives.length >= limit)
                        break;
                    ...
                    const transformedReps = limitedReps.map((rep) => {
                        // Construct name from given_name and family_name since OpenStates doesn't have a single name field
                        let fullName = '';
                        if (rep.given_name && rep.family_name) {
                            fullName = `${rep.given_name} ${rep.family_name}`;
     ...
     */
    getOfficeFromRoles(roles) {
        if (!roles || roles.length === 0)
            return 'Representative';
        const currentRole = roles.find(role => role.current === true);
        if (currentRole) {
            if (currentRole.type === 'upper')
                return 'Senator';
            if (currentRole.type === 'lower')
                return 'Representative';
            if (currentRole.type === 'executive')
                return 'Governor';
        }
        return 'Representative';
    }
     ...
     */
    getOfficeFromRoleType(roleType) {
        const roleMap = {
            'upper': 'Senator',
...
//# sourceMappingURL=superior-data-pipeline.js.map