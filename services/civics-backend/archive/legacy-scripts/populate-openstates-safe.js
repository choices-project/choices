...
// Configuration
const OPENSTATES_DATA_PATH = path.join(__dirname, '../data/openstates-people/data');
...
async function processPersonFile(filePath, stateCode) {
    try {
        ...
        const currentRelevantRole = personData.roles?.find(role =>
            ['upper', 'lower', 'governor', 'lt_governor', 'mayor'].includes(role.type) &&
            (!role.end_date || new Date(role.end_date) > new Date())
        );
