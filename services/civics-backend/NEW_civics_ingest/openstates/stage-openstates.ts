...
interface RawPerson {
  ...
  other_identifiers?: RawIdentifier[];
  roles?: RawRole[];
  sources?: RawSource[];
...
function extractIdentifiers(person: RawPerson): StagedPerson['identifiers'] {
  ...
  const pushIdentifier = (
    ...
    const key = `${scheme.toLowerCase()}:${identifier}`;
  ...
  if (Array.isArray(person.identifiers)) {
    for (const entry of person.identifiers) {
      pushIdentifier(entry.scheme, entry.identifier, entry.start_date ?? null, entry.end_date ?? null);
    }
  }

  if (Array.isArray(person.other_identifiers)) {
    for (const entry of person.other_identifiers) {
      pushIdentifier(entry.scheme, entry.identifier, entry.start_date ?? null, entry.end_date ?? null);
    }
  }
  ...
  if (person.ids && typeof person.ids === 'object') {
    for (const [scheme, value] of Object.entries(person.ids)) {
      pushIdentifier(scheme, typeof value === 'string' ? value : String(value), null, null);
    }
  }
...
async function replaceChildRows<T extends { openstates_person_id: number }>(
  ...
  for (let i = 0; i < rowChunks.length; i++) {
    ...
    await withRetry(async () => {
      const { error } = await client.from(table).insert(batch, insertOptions);
      if (error) {
        throw error;
      }
